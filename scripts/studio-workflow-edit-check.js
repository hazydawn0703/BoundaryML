...
assert(server.includes("!['created', 'cancelled'].includes(existingSession.status)") && app.includes("projectAgent: { open: true, request: '', session: null, pending: false, progress: null }"), 'project creation agent does not reuse terminal created/cancelled sessions for future projects');
assert(app.includes('project-agent-workspace') && app.includes('project-agent-composer') && app.includes('toggle-project-agent-drawer'), 'project creation should reuse the Studio Agent drawer and composer interaction');
assert(server.includes('isCancelProjectCreationRequest') && server.includes('cancelProjectCreationSession') && app.includes('clear-project-agent'), 'project creation agent supports cancelling the current creation session');
