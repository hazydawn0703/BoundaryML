...
assert(server.includes("!['created', 'cancelled'].includes(existingSession.status)") && app.includes("projectAgent: { request: '', session: null, pending: false }"), 'project creation agent does not reuse terminal created/cancelled sessions for future projects');
...
assert(app.includes('apiClient.projectAgentApi.message') && app.includes('project_creation_session') && app.includes('apiClient.workflowApi.generate(createdProject.id)'), 'studio creates projects through agent and generates workflow drafts');
assert(app.includes('renderProjectAgentMessage') && app.includes('Agent is thinking...') && styles.includes('.project-agent-message.user'), 'create project agent renders multi-turn chat history and pending state');
assert(app.includes('renderProjectAgentMessage(message, language') && app.includes("translateText(role === 'user' ? 'You' : 'Agent'") && app.includes("translateText('Missing:', language)") && app.includes("translateText(agent.pending ? 'Creating...' : 'Create Project with Agent'"), 'create project agent renders chat controls in the active UI language');
