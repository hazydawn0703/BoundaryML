...
- smoke:server: pass (`npm run smoke:server`)
- check: pass (`npm run check`)

### Job Acceptance
- Passed: generation endpoints create/query jobs; jobs include `input_snapshot`; idempotency verified on context summarize; retry/cancel APIs callable; restart后 jobs 可读取。
- Issues: 部分生成路径（如 execution kit）在当前 mock 数据下可能失败，需在 Phase 3 继续完善稳定性与失败分类覆盖。

### History / Snapshot Acceptance
- Passed: workflow generate/patch/node patch/diff apply/mark-final 均推进 version 并记录 history/snapshot；history/version API 可读；重启后可读取。
- Issues: 历史项语义仍偏轻量，后续可补更细粒度 summary 与关联追踪。

### Undo / Restore / Revert Acceptance
- Passed: undo/restore/revert API 可用，均创建新 version，不删除历史，并重跑 validation。
- Issues: undo 策略当前为“回到上一可用快照”P0语义，尚未扩展更复杂选择。

### Storage Integrity Acceptance
- Passed: FileStorage atomic write；corrupted JSON 错误码；migration skeleton 已加；关键写接口支持 optimistic lock 并返回 `VERSION_CONFLICT`。
- Issues: 未新增独立破坏性注入测试脚本，当前主要依赖集成验证。

### Execution Kit Acceptance
- Passed: kit 生成路径已加 status/download guard；workflow 变化会把旧 kit 标记 stale；download 返回 stale 标记。
- Issues: “generating->generated/failed”完整状态机仍是同步简化实现，需后续异步队列化。

### API / Protocol Consistency
- Passed: envelope 统一（`ok/data/meta.requestId` / `ok=false,error.code`）；smoke 覆盖多路由；snake_case 继续作为主协议。
- Issues: 仍有历史 camelCase 兼容字段残留（仅读兼容），Phase 3 可继续收敛。

### Remaining Known Limitations
- Job 与 Kit 执行仍为同步路径。
- corruption/schema/version 的负向场景缺少独立自动化用例矩阵。
