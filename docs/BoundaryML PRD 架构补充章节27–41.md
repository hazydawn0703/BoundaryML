...
Execution Dispatch Layer 是 BoundaryML 与受控外部执行之间的调度边界。职责包括：
...
Agent Adapter Layer 是 BoundaryML 与外部 Agent / 工程工具的适配层。职责包括：

- 将 BoundaryML Agent Task 转换成外部 Agent 的输入格式；
