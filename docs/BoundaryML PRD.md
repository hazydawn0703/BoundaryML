...
BoundaryML 是面向 **人机协作工作流** 的边界建模与治理层。
...
BoundaryML 的第一个重点落地场景是 **Agentic Development**。在这个场景中，BoundaryML 将模糊的软件开发任务转成可审核、可测试、可交接给 Codex / Claude Code / GitHub Copilot / Cursor / Hermes / OpenClaw 等 Coding Agent 的 Agent-ready Execution Kit。

BoundaryML 不是项目管理工具本身，也不是 AI Coding Agent 或 Agent Runtime。它是 AI / Agent 执行前的 **人机分工蓝图层、任务边界层和治理层**。

BoundaryML 不做通用 Coding Agent Runtime，不直接替代 Codex、Claude Code、Cursor、Hermes、OpenClaw 等 Agent，也不直接把沙箱环境推到生产环境。BoundaryML 负责定义 AI / Agent 能做什么、访问什么、花多少钱、必须通过哪些测试、谁审核、如何进入正式工程链路。Agentic Development 场景中的核心原则是：
...
Agent Execution Plan 表示一个 Node 是否可以交给外部 Agent 执行，以及交给哪个 Agent、以什么模式执行。它不表示 BoundaryML 自身成为执行运行时，而是把执行边界、执行等级、目标 Adapter 和人工确认要求结构化。
...
Agent Adapter 是 BoundaryML 与外部 Agent / 工程工具之间的适配层，例如 Codex Adapter、Claude Code Adapter、Hermes Adapter、OpenClaw Adapter、GitHub PR Adapter、Jira / Linear Adapter。Adapter 只做格式转换和受控交接，不在 Adapter 中绕过 Boundary Rules。
...
BoundaryML 的主链路应体现 Agent-ready 语义：
...
Execution Kit 是 BoundaryML 的核心产出包。在对外交接语义上，它应被视为 Agent-ready Execution Kit：用于把已确认的 workflow、node boundary、prompt、review gate 和 artifact contract 交给 Codex / Claude Code / GitHub Copilot / Cursor / GitHub Issues 或人工审核者。
...
# 系统架构、前后端边界、通信层、持久化层和 LLM 接入层详见《BoundaryML PRD 架构补充章节：27–41》。后续开发不得将 BoundaryML 仅实现为纯前端工程；Local Demo Mode 只作为演示模式存在，正式开源运行模式应包含 BoundaryML Server、Persistence Layer 和 LLM Access Layer。
