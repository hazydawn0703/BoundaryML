...
BoundaryML 是 AI Agent 时代的 **项目启动与治理层**，用于在 Codex / Claude Code / GitHub Copilot / Cursor 等 AI agents 执行前，将模糊项目转成可执行、可审核、可交接的 **Agent-ready Execution Kit**。

产品内核仍然是面向 AI 转型项目的 **可视化人机协作边界编排系统**。它帮助团队在项目启动和落地前，明确：
...
BoundaryML 不是项目管理工具本身，也不是 AI coding agent 或 Agent runtime，而是 agents 执行前的 **人机分工蓝图层、任务边界层和治理层**。

---

## 1. 背景与问题

AI coding agents 已经越来越能执行任务，传统软件公司在 AI 转型过程中，常见问题也从“是否使用 AI”转向“如何把任务准备到适合 AI 执行、适合人工审核、适合项目交接的程度”。真实团队仍然面临：

1. 任务描述过粗，Agent 容易跑偏；
2. 上下文不完整，Agent 会补错；
3. 输出格式不清晰，产物难交接；
4. 验收标准缺失，PR / 文档 / 任务难 review；
5. 高风险节点缺少人工审核门；
6. 多个 AI 工具之间缺少统一的任务边界和交接结构；
7. 不知道项目全流程中哪些节点适合 AI 介入，哪些节点必须人工兜底；
8. 不知道如何把人机分界结果同步到开发、测试、项目管理和审批流程中。

结果是 AI 使用碎片化，团队成员各自为战：产品用 AI 写 PRD，开发用 AI 写代码，测试用 AI 写用例，但整个项目没有一张全局的人机协作边界图，也没有一套可交给 Codex / Claude Code / GitHub Copilot / Cursor / GitHub Issues / 人工审核者的统一任务包。
...
- 支持生成 Agent-ready Execution Kit；
- 支持将项目拆成适合 Codex / Claude Code / GitHub Copilot / Cursor 执行的任务包；
...
| Dev agency / delivery team | 为客户项目生成可交接的 workflow、prompt pack 和 review checklist |
| AI transformation consultant | 为客户生成 AI 转型项目落地蓝图与治理边界 |
| Enterprise AI governance owner | 沉淀组织级 AI 使用规范、人机协作模板和审批规则 |
