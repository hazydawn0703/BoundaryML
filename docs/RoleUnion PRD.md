...
> 文档定位：本文用于定义 RoleUnion 的产品边界、核心工作流与关键功能要求。本文不替开发定义数据库表结构，不强行规定技术实现方式；重点说明产品目标、用户行为、输入输出、页面能力、交互逻辑与验收口径。

---

## 0. 产品一句话定义

RoleUnion 是面向 **人机协作工作流** 的边界建模与治理层。

它帮助团队在 AI 参与复杂项目之前，明确：

- 项目从立项到交付有哪些阶段和节点；
- 每个节点需要什么输入、产出什么交付物；
- 每个节点应由人工、AI，还是人工 + AI 执行；
- 哪些节点可以交给外部 Agent 或 AI 工具；
- AI / Agent 执行节点需要什么提示词、上下文、权限边界、成本约束和验收规则；
- 人工审核节点应如何确认 AI 产物是否可以进入下游；
- 最终如何导出可交接、可审核、可追踪的 Execution Kit。

RoleUnion 的第一个重点落地场景是 **Agentic Development**。在这个场景中，RoleUnion 将模糊的软件开发任务转成可审核、可测试、可交接给 Codex / Claude Code / GitHub Copilot / Cursor / Hermes / OpenClaw 等 Coding Agent 的 Agent-ready Execution Kit。

RoleUnion 不是项目管理工具本身，也不是 AI Coding Agent 或 Agent Runtime。它是 AI / Agent 执行前的 **人机分工蓝图层、任务边界层和治理层**。

RoleUnion 不做通用 Coding Agent Runtime，不直接替代 Codex、Claude Code、Cursor、Hermes、OpenClaw 等 Agent，也不直接把沙箱环境推到生产环境。RoleUnion 负责定义 AI / Agent 能做什么、访问什么、花多少钱、必须通过哪些测试、谁审核、如何进入正式工程链路。Agentic Development 场景中的核心原则是：
...
AI 正在进入软件开发、内容生产、运营交付、数据分析、企业审批和自动化流程。问题不再只是“是否使用 AI”，而是“如何把工作准备到适合 AI 执行、适合人工审核、适合团队交接、适合风险治理的程度”。真实团队仍然面临：

1. 任务描述过粗，AI / Agent 容易跑偏；
2. 上下文不完整，AI 会补错；
3. 输出格式不清晰，产物难交接；
4. 验收标准缺失，文档 / PR / 报告 / 任务难 review；
5. 高风险节点缺少人工审核门；
6. 多个 AI 工具之间缺少统一的任务边界和交接结构；
7. 不知道项目全流程中哪些节点适合 AI 介入，哪些节点必须人工兜底；
8. 不知道如何把人机分界结果同步到开发、运营、内容、数据、项目管理和审批流程中。

在 Agentic Development 场景中，这个问题尤其明显：Coding Agent 越来越能读代码、改代码、运行测试和生成 PR，但如果缺少任务边界、上下文范围、输出契约、测试门和 Review Gate，Agent 执行结果仍然难以进入正式工程链路。

结果是 AI 使用碎片化，团队成员各自为战：产品用 AI 写 PRD，开发用 AI 写代码，运营用 AI 写活动方案，数据团队用 AI 写分析报告，但整个项目没有一张全局的人机协作边界图，也没有一套可交给下游 AI 工具、Agent、项目管理系统或人工审核者的统一任务包。

RoleUnion 要解决的是这个上游问题。

---

## 2. 产品目标

RoleUnion 的目标不是让 AI 尽可能多地接管项目，而是让 AI 参与项目时变得 **可规划、可审核、可交接、可追踪**。
...
- 支持通用人机协作 Workflow 的建模、校验和导出；
- 支持将特定场景的 Workflow 转换为场景化 Execution Kit；
- 第一阶段重点支持 Agentic Development 的 Agent-ready Execution Kit；
- 支持为 Coding Agent / Agentic Development 生成执行前治理契约；
- 支持将软件开发项目拆成适合 Codex / Claude Code / GitHub Copilot / Cursor / Hermes / OpenClaw 执行的任务包；
- 支持为每个 AI 参与节点生成 Prompt、Required Context、Output Contract、Acceptance Criteria 和 Review Gate；
- 支持导出给 GitHub Issues / Markdown / YAML / JSON，后续扩展到 Linear / Jira；
- 支持将最终蓝图导出为任务清单、提示词包、审核清单和交付物要求。

### 2.2 非目标

RoleUnion 当前阶段不做以下事情：

- 不替代 Jira、飞书项目、Linear、禅道等项目管理工具，而是为这些工具生成可交接任务包；
- 不替代 Codex / Claude Code / GitHub Copilot / Cursor 等 AI coding agents；
- 不作为 Agent runtime；
- 不作为通用流程图工具；
- 不作为通用自动化平台；
- 不自动执行生产环境任务；
...
- 可以通过后续 Agent Adapter 将已确认的节点交接给外部 Coding Agent，但 MVP 不直接执行代码修改；
- 不直接部署生产，不绕过 Git / CI / Review Gate；
- 输出的是 Agent 可消费的 Execution Contract、Prompt、Checklist、测试门、发布门和 Evidence 要求。
...
| 用户角色 | 主要诉求 |
|---|---|
| 产品经理 | 将复杂项目从目标拆成清晰流程，并明确哪些环节可由 AI 辅助 |
| 项目经理 | 明确任务流、责任人、审核点和交付物 |
| 运营负责人 | 设计活动、增长、GEO、内容分发等流程中的人机分工与审核机制 |
| 内容负责人 | 治理内容生产、审核、发布、复盘流程中的 AI 参与边界 |
| 咨询顾问 / 交付团队 | 为客户生成可交接的 workflow、execution kit 和 review checklist |
| 企业 AI 转型负责人 | 沉淀组织级 AI 使用规范、人机协作模板和审批规则 |
| 数据分析负责人 | 明确数据分析、报告生成、洞察审核和下游决策的 AI 使用边界 |
| 风控 / 合规 / 安全负责人 | 定义高风险节点、Review Gate、权限边界和发布门禁 |
| AI coding adopter | 把模糊需求转成可交给 Codex / Claude Code / Copilot / Cursor 的任务包 |
| SaaS founder | 将 MVP / feature idea 快速拆成可执行、可审核的 agent-ready plan |
| Engineering Lead | 判断哪些工程节点可以交给 AI，哪些必须人工把关，并定义 Review Gate |
...
### 3.3 第一个重点场景：Agentic Development

- AI SaaS 功能从 0 到 1 开发；
- 传统系统 AI 化改造；
- 内部工具接入 AI 能力；
- 使用 AI Coding 工具重构遗留系统；
- 建立企业内部 AI Coding 交付规范；
- 企业使用 Coding Agent 进行功能开发、Bugfix、测试补充、重构和代码审查前的执行边界规划；
- 为 Codex / Claude Code / Cursor / GitHub Copilot / Hermes / OpenClaw 生成可审核任务包；
- 为 Agent 在沙箱中执行任务定义 repo scope、runtime scope、secret scope、test gate 和 promotion policy。
...
Project 是 RoleUnion 的顶层对象，代表一个待规划、待交付或待改造的项目。
...
Context Pack 是 RoleUnion 生成工作流和判断人机分界时使用的背景信息集合。
...
### 4.11 Agent Execution Plan

Agent Execution Plan 表示一个 Node 是否可以交给外部 Agent 执行，以及交给哪个 Agent、以什么模式执行。它不表示 RoleUnion 自身成为执行运行时，而是把执行边界、执行等级、目标 Adapter 和人工确认要求结构化。

### 4.12 Sandbox Execution Contract

Sandbox Execution Contract 表示一个可交给 Coding Agent 的开发节点在沙箱中执行时的完整约束，包括仓库范围、分支范围、允许路径、禁止路径、命令白名单、密钥策略、网络策略、成本预算、测试要求、输出证据和发布策略。

### 4.13 Agent Adapter

Agent Adapter 是 RoleUnion 与外部 Agent / 工程工具之间的适配层，例如 Codex Adapter、Claude Code Adapter、Hermes Adapter、OpenClaw Adapter、GitHub PR Adapter、Jira / Linear Adapter。Adapter 只做格式转换和受控交接，不在 Adapter 中绕过 Boundary Rules。

### 4.14 Execution Evidence

Execution Evidence 是 Agent 执行完成后必须回传或生成的证据，包括 code diff、commit、test report、build log、preview URL、risk summary、cost report、rollback note 等。

### 4.15 Promotion Gate

Promotion Gate 是从 sandbox 到 PR、从 PR 到 staging、从 staging 到 production 的门禁规则。Promotion Gate 不等于 Review Gate，但通常会依赖 Review Gate、测试门和权限门。

---

## 5. 模型接入层 Model Access Layer

RoleUnion 的核心蓝图不是由用户手工从零绘制出来的，而是由 LLM 基于项目输入、组织上下文、审批约束和模板规则生成初始人机分工蓝图。

因此，RoleUnion 必须定义一个明确的模型接入层，用于承接以下能力：
...
模型接入层不是一个隐藏的“调用大模型工具函数”，而是 RoleUnion 的核心产品能力之一。
...
RoleUnion 中的 LLM 输出默认是建议状态，而不是最终状态。
...
为了让 RoleUnion 的结果可审计，系统应记录必要的模型调用摘要。
...
### 6.1 RoleUnion 的初始输入是否需要组织架构和现有审批流程？

结论：**需要支持，但不应作为强制启动条件。**

RoleUnion 的最小启动输入应该是：
...
但如果用户提供组织架构和现有审批流程，RoleUnion 应该能生成更贴近真实执行的蓝图。
...
RoleUnion 的可视化界面不应该只依赖对话修改，也不应该只依赖拖拽修改。
...
如果没有拖拽，RoleUnion 会更像一个 AI Chat 工具，而不是可视化编排系统。
...
RoleUnion 的主链路应体现 Agent-ready 语义：

```text
Project goal / PRD / GitHub issue backlog
  → Context Pack
  → Workflow
  → Node boundary
  → Prompt Pack
  ...
  → Codex / Claude Code / GitHub Issues / Human Review
```

### 7.1 创建项目

用户进入 RoleUnion 后，创建一个新项目。
...
Execution Kit 是 RoleUnion 的核心产出包。在对外交接语义上，它应被视为 Agent-ready Execution Kit：用于把已确认的 workflow、node boundary、prompt、review gate 和 artifact contract 交给 Codex / Claude Code / GitHub Copilot / Cursor / GitHub Issues 或人工审核者。

它包括：

- Agent Task List / 任务清单；
- Prompt Pack；
...
RoleUnion 应支持将 Execution Kit 导出为：
...
- 选择导出目标：Markdown / YAML / JSON / GitHub Issues。
...
1. RoleUnion 的核心蓝图由 LLM 基于 Project、Context Pack、Template 和 Boundary Rules 生成；
2. RoleUnion 的初始输入不只是任务目标，还可以包含 Context Pack；
...
6. RoleUnion 的核心产出是 Execution Kit，包括任务、提示词、审核清单、交付物模板和风险报告；
...
# 章节 15–26 已整理为独立补充文件《RoleUnion PRD 补充章节：15–26》，用于定义 RoleUnion Spec、Boundary Rules、风险策略、状态机、模板系统、Execution Kit、Diff 规则、Context Pack、Prompt 版本与 MVP 验收标准。

# 系统架构、前后端边界、通信层、持久化层和 LLM 接入层详见《RoleUnion PRD 架构补充章节：27–41》。后续开发不得将 RoleUnion 仅实现为纯前端工程；Local Demo Mode 只作为演示模式存在，正式开源运行模式应包含 RoleUnion Server、Persistence Layer 和 LLM Access Layer。
