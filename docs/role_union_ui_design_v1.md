# RoleUnion UI 设计方案 v0.1

> 适用范围：本文基于《RoleUnion PRD v0.1》产出，用于指导 MVP 阶段的前端页面设计、交互设计与视觉组件设计。本文不定义数据库结构，不限定具体前端技术栈，重点定义用户界面、信息架构、页面布局、组件状态和关键交互。

---

## 1. UI 设计目标

RoleUnion 的 UI 不应该做成普通项目管理后台，也不应该做成纯 AI Chat 工具。
...
让用户快速进入已有项目，或创建一个新的 RoleUnion 项目。
...
RoleUnion will generate a draft workflow, recommend human-AI execution modes, and help you export an execution kit.
...
原因：RoleUnion 表达的是项目生命周期，不是任意知识图谱。
...
这是 RoleUnion 区别于普通流程图工具的核心页面之一。
...
RoleUnion 的 UI 设计不应该追求“AI 自动替你完成一切”的幻觉。
