# RoleUnion PRD 架构补充章节27–41

> 文档定位：本文作为《RoleUnion PRD》的架构补充文件，用于明确 RoleUnion 不是单纯前端工程，而是由 Studio 前端、Server 后端、Core Engine、Persistence Layer、LLM Access Layer、Communication Layer 共同组成的系统。后续开发必须以 PRD 为真源，不得仅根据当前代码形态或 MVP 临时实现反推产品架构。其中27–36章节为：系统架构与运行边界；37–41章节为：P0 可靠性、安全性与数据完整性补充。
...
RoleUnion 当前 PRD 已经定义了产品对象、规则、状态机、模板系统、Diff 机制、Execution Kit 和 MVP 验收标准，但尚未明确系统架构形态。

本章用于补齐一个关键判断：

> RoleUnion 不是一个纯前端应用，而是一个由前端 Studio、后端 Server、核心引擎、持久化层、LLM 接入层和前后端通信层组成的产品系统。
...
RoleUnion Server
  ↓
Core Engine / Boundary Rules / Generators / Persistence / LLM Access
  ↓
...
Execution Dispatch Layer 负责把已确认的 Node / Sandbox Execution Contract 转换为可执行任务。Agent Adapter Layer 负责对接外部 Agent 或工程工具。MVP 不要求真实 Adapter，但架构必须预留。

### 27.2.1 RoleUnion Studio

RoleUnion Studio 是前端可视化工作台。
...
### 27.2.3 RoleUnion Server

RoleUnion Server 是后端运行时。
...
- 调用 Execution Dispatch Layer 创建受控 Agent Run 记录；
- 返回结构化结果给 Studio。

Server 是 RoleUnion 正式能力的入口，不应被前端绕过。

---

### 27.2.4 Core Engine

Core Engine 是 RoleUnion 的产品内核。
...
- Risk Report 生成；
- Execution Kit 生成；
- Agent Execution Plan 生成；
- Sandbox Execution Contract 生成；
- Promotion Gate 生成。
...
- 保存 Execution Kit；
- 保存 Agent Execution Plan；
- 保存 Sandbox Execution Contract；
- 保存 Agent Run；
- 保存 Execution Evidence；
- 保存 Promotion Gate；
- 保存 Agent Adapter Config Reference；
- 保存 Model Config 引用；
- 保存 Model Call Log 摘要；
- 支持后续从本地文件、SQLite、Postgres 等不同存储方式切换。

### 27.2.9 Execution Dispatch Layer

Execution Dispatch Layer 是 RoleUnion 与受控外部执行之间的调度边界。职责包括：

- 读取 Node、Execution Mode、Agent Execution Level、Sandbox Execution Contract；
- 检查是否满足 Boundary Rules；
- 生成 Agent Task Payload；
- 执行前要求用户或策略确认；
- 创建 Agent Run 记录；
- 接收外部 Agent 返回的 Execution Evidence；
- 将 Evidence 绑定回 Node / Workflow / Execution Kit；
- 不允许绕过 Review Gate / Promotion Gate。

Execution Dispatch Layer 不等于通用 Agent Runtime。它只负责受控派发、状态记录和证据回收。

### 27.2.10 Agent Adapter Layer

Agent Adapter Layer 是 RoleUnion 与外部 Agent / 工程工具的适配层。职责包括：

- 将 RoleUnion Agent Task 转换成外部 Agent 的输入格式；
- 支持 CLI、API、GitHub PR、文件导出等不同适配方式；
- 收集 Agent 执行结果；
- 标准化 diff、test report、preview URL、cost report、risk summary；
- 不在 Adapter 中做产品规则判断，规则判断仍由 Boundary Rules 完成。

示例 Adapter 包括 Codex Adapter、Claude Code Adapter、Cursor Adapter、Hermes Adapter、OpenClaw Adapter、GitHub PR Adapter、Jira / Linear Adapter。MVP 可以只保留配置引用和导出格式，不要求真实调用。

---

## 27.3 架构原则

RoleUnion 后续开发必须遵循以下架构原则。

### 27.3.1 PRD 是开发真源

代码结构必须服从 PRD，而不是让 PRD 适配当前代码。

如果当前代码只是前端 localStorage Demo，也不能因此把 RoleUnion 定义为纯前端工具。
...
RoleUnion 需要支持从开源 Demo 到本地部署再到 SaaS 化的演进，因此 PRD 需要明确不同运行模式。
...
Local Server Mode 是 RoleUnion 的第一个正式可用开源运行模式。
...
```text
studio.company.com  → RoleUnion Studio
api.company.com     → RoleUnion Server API
```
...
RoleUnion 的运行架构必须保持清晰的 Open Core 扩展边界：
...
- Pro workflow templates、Human-AI Workflow Governance Kits、GEO Launch Ops Kit、AI Coding Governance Kit（first commercial wedge）、enterprise review gate templates 等商业资产可以在 PRD 中作为产品方向出现，但不应被默认标记为 Apache-2.0 开源资产。
...
本章明确 RoleUnion 前后端职责边界，防止前端长期承担不该承担的核心逻辑，也防止后端过度侵入前端交互。
...
为了避免前后端逻辑重复，RoleUnion 应将以下逻辑沉淀为 shared packages：
...
Communication Layer 定义 RoleUnion Studio 与 RoleUnion Server 之间的正式通信方式。
...
```env
ROLEUNION_API_BASE_URL=http://localhost:8787
```

或在构建环境中使用：

```env
VITE_ROLEUNION_API_BASE_URL=http://localhost:8787
```
...
Persistence Layer 定义 RoleUnion 如何保存项目和蓝图资产。

RoleUnion 的核心资产不是页面状态，而是可持续迭代的人机分工蓝图。

---

## 31.2 需要持久化的对象

RoleUnion 至少需要持久化以下对象：
...
| Template | 是 | 内置或组织模板 |
| Agent Execution Plan | 是 | Node 到 Agent 执行等级和目标 Adapter 的计划 |
| Sandbox Execution Contract | 是 | Agent 沙箱执行约束 |
| Agent Run | P1/P2 | 外部 Agent 运行记录，P0 可仅导出草稿 |
| Execution Evidence | P1/P2 | diff、test report、preview URL、cost report、risk summary 等证据 |
| Promotion Gate | 是 | sandbox → PR → staging → production 的门禁规则 |
| Agent Adapter Config Reference | 是 | Adapter 配置引用，不含生产密钥明文 |

不得保存生产密钥明文。`secret_scope` 只能保存策略、引用或环境 profile，不应保存实际 secret value。
...
RoleUnion 的模型调用必须可配置、可替换、可降级、可审计，并且不能泄露 API Key。
...
```env
ROLEUNION_SERVER_PORT=8787
ROLEUNION_API_BASE_URL=http://localhost:8787

ROLEUNION_STORAGE_ADAPTER=file
ROLEUNION_DATA_DIR=./data
...
ROLEUNION_LLM_TIMEOUT_MS=60000
ROLEUNION_LLM_ENABLE_STRUCTURED_OUTPUT=true
ROLEUNION_LLM_LOG_LEVEL=summary
...
本章定义 RoleUnion 的部署拓扑，确保产品设计不被单机 MVP 限制。
...
```text
https://studio.roleunion.dev
https://api.roleunion.dev
```
...
它不是要求当前立即实现完整 SaaS，而是确保 RoleUnion 不再被误解为纯前端工程。

---

## 34.2 PRD 层验收标准

1. PRD 明确 RoleUnion 由 Studio、Communication Layer、Server、Core Engine、Persistence Layer、LLM Access Layer 组成；
...
> 当前版本是 Local Demo Mode 的雏形，不是 RoleUnion 的完整系统架构。
...
RoleUnion 即使在早期不实现完整用户 CRUD，也必须定义最小数据隔离模型。

原因是 RoleUnion 持久化的数据包含项目目标、组织上下文、审批流程、提示词、模型调用摘要、执行包等敏感资产。如果不同用户或不同组织的数据没有隔离，后续无法安全演进为 Local Server、Self-hosted 或 SaaS 形态。
...
| 概念 | 说明 |
|---|---|
| User | 操作 RoleUnion 的个体身份，可以先是本地默认用户 |
...
RoleUnion 的初始蓝图由 LLM 生成，但 LLM 输出不能是自由文本，也不能是 Markdown 描述。

生成初始蓝图、Workflow Diff、Prompt Asset、Checklist Asset 等核心对象时，LLM 必须输出符合 RoleUnion Spec 的标准 JSON。
...
2. JSON 必须符合 RoleUnion Workflow Draft Schema。
...
RoleUnion 的持久化 JSON 和 Server API 返回对象应采用统一字段命名。

建议使用 snake_case，与 RoleUnion Spec 保持一致：
...
```json
{
  ...
  "error": {
    ...
    "message": "The model output does not match RoleUnion Workflow Draft Schema.",
    "details": []
  }
}
```
...
The JSON must match RoleUnion Workflow Draft Schema.
...
RoleUnion 的核心能力依赖多个生成动作，包括 Workflow Draft 生成、Context Pack 摘要生成、Prompt 生成、Checklist 生成、Workflow Diff 生成和 Execution Kit 生成。

这些生成动作可能涉及 LLM 调用、结构化输出解析、Schema 校验、Boundary Rules 校验、持久化写入和导出文件生成，不能被简单视为一次同步请求。

本章定义 RoleUnion 的 Generation Job 机制，用于保证生成过程可追踪、可恢复、可重试、可展示状态，并避免重复点击、超时、刷新页面或服务重启造成数据状态混乱。
...
| Generate Agent Execution Plan | 是 |
| Generate Sandbox Execution Contract | 是 |
| Generate Promotion Gate | 是 |
| Dispatch Agent Run | P1/P2 |
| Collect Execution Evidence | P1/P2 |
| Generate Artifact Template | 建议 |
| Generate Risk Report | 建议 |
| Generate Responsibility Map | 建议 |

MVP 阶段允许部分生成动作在实现上仍为同步执行，但产品语义上必须创建 Job，并返回 `job_id` 与最终状态。P0 可以只生成 Contract，不实际派发 Agent；P1/P2 再做 dispatch 和 callback。
...
| `generate_agent_execution_plan` | 生成 Agent Execution Plan |
| `generate_sandbox_execution_contract` | 生成 Sandbox Execution Contract |
| `generate_promotion_gate` | 生成 Promotion Gate |
...
| `generate_agent_execution_plan` | `agent_execution_plan_id` |
| `generate_sandbox_execution_contract` | `sandbox_execution_contract_id` |
| `generate_promotion_gate` | `promotion_gate_id` |

Job 不应只返回一段文本结果。

所有生成结果必须进入 RoleUnion Spec 对应对象，并经过必要校验。
...
Reason: Model output did not match RoleUnion Workflow Draft Schema.
...
Agent Run 记录一次外部 Agent 受控执行的生命周期。P0 可以只产生 `draft` 或导出对象，P1/P2 再进入真实派发状态。
...
RoleUnion 的 Workflow 是高价值项目资产。用户会在 Studio 中手动编辑节点、应用 AI Diff、修改执行模式、调整 Review Gate、生成 Prompt 和导出 Execution Kit。

如果用户误操作、AI Diff 应用错误、节点被误删或上下文刷新造成错误变更，系统必须提供可恢复能力。

本章定义 Workflow History、Undo 和 Restore 规则，确保 RoleUnion 的编辑体验可控、可回退、可审计。
...
RoleUnion 会使用用户输入的 Context Pack、历史流程材料、审批规则、组织文档和项目说明来生成 Workflow、Review Gate、Prompt 和 Execution Kit。
...
Agent 不能因为 prompt 或任务描述而覆盖 Boundary Rules。
外部 Agent 返回的结果必须被当作 untrusted output。
Execution Evidence 必须经过 schema validation 和 Boundary Rules validation。
Agent 不得默认访问生产密钥。
外部网络访问必须显式授权。
shell command 必须基于 allowlist。
sandbox 不得默认拥有 production deploy 权限。
suspicious diff / forbidden path change 必须阻止进入下游。
...
RoleUnion will treat it as reference material, not as system instruction.
...
RoleUnion 已定义 User、Workspace、Project Owner、Request Context 和 Demo Workspace 等概念，并要求所有正式 Project 数据必须归属于 Workspace。

但仅有 workspace_id 字段还不够。Server 必须知道每次 API 请求属于哪个 User 和 Workspace，才能正确执行数据隔离、查询过滤、写入归属和审计记录。

本章定义 RoleUnion 的最小 Request Context 解析规则。
...
```text
ROLEUNION_DEFAULT_USER_ID=local_user
ROLEUNION_DEFAULT_WORKSPACE_ID=local_default
```
...
X-RoleUnion-Workspace: workspace_001
X-RoleUnion-User: user_001
...
RoleUnion 的核心数据包括 Project、Context Pack、Workflow、Node、Edge、Prompt、Checklist、Workflow Diff、Validation Result、Execution Kit 和 Model Call Log。
...
Agent Run 成功不等于 Node 完成。
Evidence 保存失败不得标记 Agent Run succeeded。
Promotion Gate 未通过不得标记 promoted。
sandbox 执行失败不得影响当前正式 Workflow。
Agent Run 必须绑定 input snapshot。
Agent Run 输出必须绑定 output evidence。
Agent Run 不得直接覆盖 Workflow，只能生成 Diff / Evidence / PR。
...
当 RoleUnion Spec 或 Schema 升级时，系统必须有迁移策略。
