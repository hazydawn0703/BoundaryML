...
> 文档定位：本文作为《BoundaryML PRD》的补充文件，专门补齐产品内核层尚未明确定义的规则、状态、规格与输出协议。本文不定义数据库表结构，不限定具体前端技术栈或后端实现方式；重点定义产品对象、校验规则、生命周期、导出结构与 MVP 验收标准。

---

# 15. BoundaryML Spec v0.1
...
如果没有统一 Spec，前端、模型输出、导出文件和规则校验会各自定义一套结构，导致 BoundaryML 很快变成不可维护的流程图工具。

---

## 15.2 Spec 设计原则

BoundaryML Spec 应遵循以下原则：
...
BoundaryML 项目级 Spec 应至少包含以下顶层对象：
...
BoundaryML 不应该只让用户手动选择 Low / Medium / High，而应根据项目和节点信息生成风险建议。
...
本章定义 BoundaryML 核心对象的状态机，避免 Draft、Reviewed、Applied、Final、Outdated 等状态被混用。
...
Workflow Template 是 BoundaryML 快速生成高质量初始蓝图的基础。

BoundaryML 不应该让用户从空白画布开始，也不应该完全依赖 LLM 自由生成。模板系统用于提供稳定结构，LLM 用于结合具体项目上下文进行裁剪、补充和解释。
...
| 类型 | 说明 |
|---|---|
| Built-in Template | BoundaryML 内置模板 |
...
Execution Kit 是 BoundaryML 的核心外部交付物。在对外产品语义中，它应被明确描述为 **Agent-ready Execution Kit**。

它的目标不是让 BoundaryML 自己执行任务，而是让团队把人机分工蓝图、任务边界、Prompt、Required Context、Output Contract、Acceptance Criteria 和 Review Gate 交接给 Codex / Claude Code / GitHub Copilot / Cursor / GitHub Issues / 人工审核者和交付团队。
...
本章用于明确 BoundaryML 的产品边界，防止项目从“人机分工蓝图系统”膨胀成“全流程项目管理 + AI 执行平台”。

BoundaryML prepares work before agents execute. 它不替代 Codex / Claude Code / GitHub Copilot / Cursor，也不替代项目管理工具，而是在这些工具执行或承接任务之前定义任务边界、上下文、输出契约、Review Gate 和验收标准。

---

## 21.2 核心边界

BoundaryML 默认不直接执行项目工作，也默认不执行真实项目任务。
...
BoundaryML 中的 Execution Mode 表达的是：

> 该节点在真实项目中建议采用的人机协作方式。

它不等于 BoundaryML 自身会执行该节点，也不表示 BoundaryML 一定会调用某个 coding agent 或自动推进真实项目任务。

例如：

| Execution Mode | 在 BoundaryML 中的含义 |
|---|---|
| AI Execute + Human Approval | BoundaryML 生成 AI 执行提示词和人工批准清单，不代表系统自动调用 AI 完成任务 |
| AI Autonomous | 该节点可被下游系统自动执行，不代表 MVP 中 BoundaryML 自动执行 |
...
AI Assisted Edit 是 BoundaryML 的关键交互能力，但 AI 不能直接修改正式 Workflow。
...
BoundaryML 必须让 Prompt 可见、可编辑、可版本化、可导出，并与 Node Contract 绑定。
...
MVP 必须提供一个内置示例 Workflow，让用户首次进入项目时快速理解 BoundaryML 的价值。
...
本章定义 BoundaryML MVP 的产品验收标准。

MVP 不要求功能完整，但必须证明 BoundaryML 不是普通流程图工具，而是一个可生成、校验、编辑和导出人机分工蓝图与 Agent-ready Execution Kit 的系统。
...
8. GitHub 用户能通过 README 和示例项目理解 BoundaryML 与普通流程图工具、Agent 框架和项目管理工具的区别；
9. README 能清晰说明 BoundaryML 是 AI Agent 项目启动与治理层；
10. 示例项目能导出 Agent-ready Execution Kit；
11. 用户能理解 BoundaryML 与 Codex / Claude Code / GitHub Copilot / Cursor 的关系；
